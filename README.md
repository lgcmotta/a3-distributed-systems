...
4. Still on the terminal run the following command:

    ```shell
    dotnet tool restore
    ```

5. Finally, if you are on **Linux** or **macOS** run the `init.sh` script or if you're in **Windows** run the `init.ps1` script.
...
6. Open the IDE of your choice and select the `AppHost` as the entry point project for debugging.
